...
export function ShareButtons({ url, title, platforms = ["whatsapp", "facebook", "twitter", "telegram", "link"], className = "" }: ShareButtonsProps) {
  ...
  const icons: Record<SharePlatform, { label: string; icon: React.ReactNode }> = {
    link: {
      label: copied ? "Copied!" : "Copy Link",
      icon: copied
        ? <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
        : <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><rect x="9" y="9" width="13" height="13" rx="2" ry="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></svg>,
    },
