...
function getBrushRadius(tone: Tone, base: number): number {
  const pacing = getAnimationDuration(tone, 100);
  const ratio = pacing / 100;
  if (tone === "Sorry" || tone === "Emotional") return Math.round(base * 0.7);
  if (tone === "Funny") return Math.round(base * 1.4);
  return Math.round(base * ratio);
}

export function ScratchCard({ template, experience, mode, shareUrl }: Props) {
  ...
  const [screen, setScreen] = useState<"intro" | "framing" | "scratch" | "holdReveal" | "reveal" | "final">("intro");
  ...
  const isScratchModeRef = useRef(false);
  const timeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const totalSteps = 6;

  const brushRadius = getBrushRadius(tone, 30);
  ...
  function computeScratchPercentage(): number {
    const canvas = canvasRef.current;
  ...
  function handlePointerDown(e: React.PointerEvent<HTMLCanvasElement>) {
    if (cleared) return;
    isScratchModeRef.current = true;
    const pos = getPointerPos(e);
    scratch(pos.x, pos.y);
  }

  function handlePointerMove(e: React.PointerEvent<HTMLCanvasElement>) {
    if (!isScratchModeRef.current || cleared) return;
    const pos = getPointerPos(e);
    scratch(pos.x, pos.y);
  }
  ...
  function handlePointerUp() {
    ...
    isScratchModeRef.current = false;
    const pct = computeScratchPercentage();
    ...
    <ExperienceLayout kicker="Playable message" theme={experience.theme} title={template.title} titleAs={mode === "generated" ? "h1" : "h2"}>
      {screen === "intro" ? (
        <StepTransition step={0}>
          <PlayerCard>
            ...
            <h2 className="display-title mt-4 text-3xl font-bold leading-tight sm:text-5xl">Scratch to reveal</h2>
            <p className="mt-5 text-white/75"><TypewriterText text="A message is hidden beneath the surface. Scratch away the silver to uncover it." /></p>
        ...
        <StepTransition step={1}>
          <PlayerCard>
            ...
            <p className="mt-3 text-sm italic text-white/50">
              There&apos;s something underneath. Are you ready to see it?
            </p>
            <button className="premium-button mt-8" type="button" onClick={() => { playSound("click"); setScreen("scratch"); }}>
              Yes, show me
            </button>
          </PlayerCard>
        </StepTransition>
        ...
        <StepTransition step={2}>
          <PlayerCard>
            ...
            <p className="text-xs font-bold tracking-[0.08em] text-white/50">
              {cleared ? "Scratched away" : `Scratch off the surface — ${Math.min(Math.round(scratchProgress), 100)}%`}
            </p>
            <div className="mt-6 flex flex-col items-center gap-4">
              ...
              {!cleared ? (
                <p className="text-sm text-white/50">Drag your finger or mouse to scratch the surface</p>
              ) : null}
            </div>
          </PlayerCard>
        </StepTransition>
      ...
      {screen === "holdReveal" ? (
        <StepTransition step={3}>
          <PlayerCard>
            <BackButton onBack={() => { setCleared(false); setScratchProgress(0); setScreen("scratch"); }} disabled={mode === "demo"} />
      ...
      {screen === "reveal" ? (
        <StepTransition step={4}>
          <PlayerCard>
            ...
            <p className="text-xs font-bold tracking-[0.08em] text-white/50">Scratched · Message delivered</p>
            <h2 className="display-title mt-4 text-3xl font-bold leading-tight sm:text-5xl">
              {experience.finalMessage}
            </h2>
